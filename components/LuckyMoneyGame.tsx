...
import { GameState, Envelope } from "../types";
import { Lock, Clock, Award, Gift } from "lucide-react";
import confetti from "canvas-confetti";
...
const LuckyMoneyGame: React.FC<Props> = ({ isVisible }) => {
  ...
  // ✅ DEV override: /page?unlocked=1 OR localStorage flag
  const devUnlocked = useMemo(() => {
    const params = new URLSearchParams(window.location.search);
    const q = params.get("unlocked") === "1";
    const ls = localStorage.getItem("lucky_money_unlocked") === "1";
    return q || ls;
  }, []);
  ...
  // ✅ Date gate + dev override
  useEffect(() => {
    const now = new Date();
    if (devUnlocked || now >= TARGET_DATE) {
      setGameState(GameState.UNLOCKED);
      initializeGame();
    } else {
      setGameState(GameState.LOCKED);
    }
  }, [devUnlocked, initializeGame]);
  ...
  const handleSelect = useCallback(
    (id: number) => {
      ...
      // ✅ redirect only if you REALLY want in prod
      if (redirectTimerRef.current) window.clearTimeout(redirectTimerRef.current);
      redirectTimerRef.current = window.setTimeout(() => {
        window.location.href = "https://www.google.com/search?q=happy+lunar+new+year+2026";
      }, 2500);
    },
    [shuffling, selectedId]
  );

  // ✅ Render rule: chỉ render khi isVisible (tránh “tự hiện” không kiểm soát)
        ...
        <div className="flex flex-col items-center gap-6 py-8">
          ...
          <div>
            <h3 className="text-2xl font-bold mb-2">
              Đây là món quà tâm huyết của tớ, hãy giữ kĩ món quà này mãi mãi nhé!
            </h3>
            <p className="opacity-70 max-w-md mx-auto">
              Quay lại vào ngày 01 tháng 01 năm 2026 (Âm lịch) để nhận lì xì từ Huyydeptrai nhaaa😎
            </p>
            ...
            {envelopes.map((env, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`
                      absolute inset-0 bg-gradient-to-br from-red-600 to-lunar-red rounded-lg border-2 border-lunar-gold shadow-xl
                      flex items-center justify-center overflow-hidden transition-opacity duration-700
                  ...
                  <div
                    className={`
                      absolute inset-0 bg-white rounded-lg border-2 border-lunar-gold
                      shadow-[0_0_30px_rgba(255,215,0,0.6)] flex flex-col items-center justify-center text-lunar-red
